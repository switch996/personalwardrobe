...
class LocalStore {
  ...
  Future<void> updateUserAvatar(String imageSourcePath) async {
    final sourcePath = imageSourcePath.trim();
    if (sourcePath.isEmpty) return;
    userAvatarPath = await _uploadImage(sourcePath);
  }
  ...
  Future<void> upsertOutfit({
    ...
    _sortInPlace();
    if (isSameDay(saved.date, DateTime.now())) {
      _syncQuickWearFromTodayOutfit();
    }
  }

  Future<void> deleteOutfit(String id) async {
    final deleted = outfits.where((e) => e.id == id).firstOrNull;
    await _api.deleteOutfit(id);
    outfits.removeWhere((e) => e.id == id);
    if (deleted != null && isSameDay(deleted.date, DateTime.now())) {
      _syncQuickWearFromTodayOutfit();
    }
  }
  ...
  Future<void> markItemAsWornToday(String closetItemId) async {
    ...
    final existing = _primaryOutfitForDay(now);
    final mergedIds = <String>[
      ...(existing?.closetItemIds ?? todaysQuickWearItemIds),
    ];

    mergedIds.remove(closetItemId);
    mergedIds.insert(0, closetItemId);
    if (mergedIds.length > _quickWearMaxItems) {
      mergedIds.removeRange(_quickWearMaxItems, mergedIds.length);
    }

    await upsertOutfit(
      existing: existing,
      imageSourcePath: existing?.imagePath ?? '',
      date: now,
      note: existing?.note ?? '',
      tags: existing?.tags ?? const <String>[],
      closetItemIds: mergedIds,
    );
  }
  ...
  Future<bool> removeQuickWearItem(String closetItemId) async {
    ...
    final updatedIds = existing.closetItemIds.where((id) => id != closetItemId).toList();
    final removed = updatedIds.length != existing.closetItemIds.length;
    if (!removed) return false;

    if (updatedIds.isEmpty && _isAutoGeneratedQuickWearOutfit(existing)) {
  ...
  void _syncQuickWearFromTodayOutfit() {
    _ensureQuickWearDate();
    final todayOutfit = _primaryOutfitForDay(DateTime.now());
    final ids = todayOutfit?.closetItemIds ?? const <String>[];
    todaysQuickWearItemIds
      ..clear()
      ..addAll(ids.take(_quickWearMaxItems));
    _pruneQuickWearItems();
  }

  OutfitEntry? _primaryOutfitForDay(DateTime day) {
    final candidates = outfitsOn(day);
    if (candidates.isEmpty) return null;
    candidates.sort((a, b) {
      final imageCompare = (b.imagePath.trim().isNotEmpty ? 1 : 0)
          .compareTo(a.imagePath.trim().isNotEmpty ? 1 : 0);
      if (imageCompare != 0) return imageCompare;
      final itemCompare = b.closetItemIds.length.compareTo(a.closetItemIds.length);
  ...
  bool _isAutoGeneratedQuickWearOutfit(OutfitEntry entry) {
    return entry.imagePath.trim().isEmpty &&
